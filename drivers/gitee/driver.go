...
func (d *Gitee) Init(ctx context.Context) error {
	...
	if token != "" {
		d.client = d.client.SetHeader("Authorization", "token "+token)
	}
	if d.Ref == "" {
		repo, err := d.getRepo()
		if err != nil {
...
func (d *Gitee) MakeDir(ctx context.Context, parentDir model.Obj, dirName string) error {
	if !d.isOnBranch {
...
func (d *Gitee) Move(ctx context.Context, srcObj, dstDir model.Obj) error {
	if !d.isOnBranch {
		return errors.New("cannot write to non-branch reference")
	}
...
func (d *Gitee) Rename(ctx context.Context, srcObj model.Obj, newName string) error {
	if !d.isOnBranch {
		return errors.New("cannot write to non-branch reference")
	}
	d.commitMutex.Lock()
...
func (d *Gitee) Copy(ctx context.Context, srcObj, dstDir model.Obj) error {
	if !d.isOnBranch {
		return errors.New("cannot write to non-branch reference")
	}
	if strings.HasPrefix(dstDir.GetPath(), srcObj.GetPath()) {
...
func (d *Gitee) Put(ctx context.Context, dstDir model.Obj, stream model.FileStreamer, up driver.UpdateProgress) error {
	if !d.isOnBranch {
		return errors.New("cannot write to non-branch reference")
	}
...
func (d *Gitee) getContentApiUrl(path string) string {
	path = utils.FixAndCleanPath(path)
	return fmt.Sprintf("https://gitee.com/api/v5/repos/%s/%s/contents%s", d.Owner, d.Repo, path)
}

func (d *Gitee) getDirectoryEntries(path string) ([]Object, error) {
	...
	var entries []Object
	if err = utils.Json.Unmarshal(body, &entries); err != nil {
		return nil, err
	}
	return entries, nil
}

func (d *Gitee) getRootTreeSha() (string, error) {
	if d.isOnBranch {
		commitSha, err := d.getBranchHead()
		if err != nil {
			return "", err
		}
		return d.getCommitTreeSha(commitSha)
	}
	sha, err := d.getCommitTreeSha(d.Ref)
	if err != nil {
		log.Warnf("failed to fetch tree sha for ref %s: %v", d.Ref, err)
		return "", nil
	}
	return sha, nil
}

func (d *Gitee) getCommitTreeSha(ref string) (string, error) {
	res, err := d.client.R().Get(fmt.Sprintf("https://gitee.com/api/v5/repos/%s/%s/git/commits/%s", d.Owner, d.Repo, ref))
	if err != nil {
		return "", err
	}
	if res.StatusCode() != 200 {
	...
	if resp.Commit.Tree.Sha != "" {
		return resp.Commit.Tree.Sha, nil
	}
	return "", fmt.Errorf("tree sha not found for ref %s", ref)
}

func (d *Gitee) get(path string) (*Object, error) {
	path = utils.FixAndCleanPath(path)
	if path == "/" {
		entries, err := d.getDirectoryEntries(path)
		if err != nil {
			return nil, err
		}
		sha, err := d.getRootTreeSha()
		if err != nil {
			return nil, err
		}
		return &Object{
...
func (d *Gitee) putBlob(ctx context.Context, s model.FileStreamer, up driver.UpdateProgress) (string, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost,
		fmt.Sprintf("https://gitee.com/api/v5/repos/%s/%s/git/blobs", d.Owner, d.Repo),
...
func (d *Gitee) renewParentTrees(path, prevSha, curSha, until string) (string, error) {
	for path != until {
...
func (d *Gitee) getTree(sha string) (*TreeResp, error) {
	res, err := d.client.R().Get(fmt.Sprintf("https://gitee.com/api/v5/repos/%s/%s/git/trees/%s", d.Owner, d.Repo, sha))
	if err != nil {
		return nil, err
	}
	if res.StatusCode() != 200 {
...
func (d *Gitee) getTreeDirectly(path string) (*TreeResp, string, error) {
	p, err := d.get(path)
	if err != nil {
		return nil, "", err
	}
...
func (d *Gitee) newTree(baseSha string, tree []interface{}) (string, error) {
	body := &TreeReq{Trees: tree}
	if baseSha != "" {
		body.BaseTree = baseSha
	}
	res, err := d.client.R().SetBody(body).
		Post(fmt.Sprintf("https://gitee.com/api/v5/repos/%s/%s/git/trees", d.Owner, d.Repo))
	if err != nil {
		return "", err
	}
	if res.StatusCode() != 201 {
...
func (d *Gitee) commit(message, treeSha string) error {
	...
	res, err := d.client.R().SetBody(body).Post(fmt.Sprintf("https://gitee.com/api/v5/repos/%s/%s/git/commits", d.Owner, d.Repo))
	if err != nil {
		return err
	}
	if res.StatusCode() != 201 {
	...
	res, err = d.client.R().
		...
		Patch(fmt.Sprintf("https://gitee.com/api/v5/repos/%s/%s/git/refs/heads/%s", d.Owner, d.Repo, d.Ref))
	if err != nil {
		return err
	}
	if res.StatusCode() != 200 {
...
func (d *Gitee) getBranchHead() (string, error) {
	res, err := d.client.R().Get(fmt.Sprintf("https://gitee.com/api/v5/repos/%s/%s/branches/%s", d.Owner, d.Repo, d.Ref))
	if err != nil {
		return "", err
	}
	if res.StatusCode() != 200 {
...
func (d *Gitee) copyWithoutRenewTree(srcObj, dstDir model.Obj) (dstSha, newSha, srcParentSha string, srcParentTree *TreeResp, err error) {
...
func (d *Gitee) getRepo() (*RepoResp, error) {
	res, err := d.client.R().Get(fmt.Sprintf("https://gitee.com/api/v5/repos/%s/%s", d.Owner, d.Repo))
	if err != nil {
		return nil, err
	}
	if res.StatusCode() != 200 {
...
func (d *Gitee) getAuthenticatedUser() (*UserResp, error) {
	res, err := d.client.R().Get("https://gitee.com/api/v5/user")
	if err != nil {
		return nil, err
	}
	if res.StatusCode() != 200 {
...
func (d *Gitee) addCommitterAndAuthor(m *map[string]interface{}) {
	if d.CommitterName != "" {
